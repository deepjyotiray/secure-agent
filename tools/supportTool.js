...
const ADMIN_PHONE  = settings.admin.number.startsWith("+") ? settings.admin.number : `+${settings.admin.number}`
...
// ── LLM support response ──────────────────────────────────────────────────────

async function askLlm(message, history, faqContext, customerContext) {
    ...
    const prompt = `You are a friendly and empathetic customer support agent for Ray's Home Kitchen, a home-cooked food delivery service.

Your job is to resolve the customer's issue using the FAQ knowledge and their order context below.

Rules:
- Be warm, empathetic, and concise
- Use the customer's name if available
- Reference their actual order details if relevant
- If you cannot resolve the issue, say so honestly and tell them to say "talk to human"
- Never make up policies or promises you cannot keep
...
async function escalateToAdmin(phone, message, history, customerContext) {
    const last10   = String(phone).replace(/@.*$/, "").replace(/\D/g, "").slice(-10)
...
async function execute(params, context, toolConfig) {
    ...
    if (isEscalationRequest(rawMessage, triggers)) {
        ...
        await escalateToAdmin(phone, rawMessage, history, customerContext)
    ...
    // Try FAQ match first (fast, no LLM)
    ...
    const response = await askLlm(rawMessage, history, faqContext, customerContext)

    if (!response) {
        // LLM failed — escalate silently and give human handoff message
        await escalateToAdmin(phone, rawMessage, history, customerContext)
        addTurn(phone, rawMessage, null)
